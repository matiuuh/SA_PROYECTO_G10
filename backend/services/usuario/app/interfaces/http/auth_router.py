...
def build_auth_router(container: Container) -> APIRouter:
    ...
    async def register(request: RegisterRequest, background_tasks: BackgroundTasks) -> AuthResponse:
        try:
    ...
    async def me(token: str = Depends(get_bearer_token)) -> AccountResponse:
        try:
    ...
    async def update_me(request: UpdateAccountRequest, token: str = Depends(get_bearer_token)) -> AccountResponse:
        try:
    ...
    async def logout(token: str = Depends(get_bearer_token)) -> MessageResponse:
        try:
